...
export const Header = forwardRef<HTMLDivElement, HeaderProps>((props, ref) => {
  ...
  const headerBlockClass = `${blockClass}--header`;
  ...
  useEffect(() => {
    if (
      ...
      const onDragStart = () => {
        if (contentRef) {
          contentRef.classList.add(`${headerBlockClass}--is-dragging`);
      ...
      const onDragEnd = () => {
        if (contentRef) {
          contentRef.classList.remove(`${headerBlockClass}--is-dragging`);
  ...
  return (
    <div ref={handleRef} className={cx(headerBlockClass, className)} {...rest}>
      {floating && (
        <Button
          kind="ghost"
          size="sm"
          ...
          hasIconOnly
          className={`${headerBlockClass}--drag-icon`}
        />
      )}
      {children}
      <Button
